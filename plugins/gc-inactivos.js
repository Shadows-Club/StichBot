...
let handler = async (m, { conn, text, args, groupMetadata }) => {
    ...
    for (let i = 0; i < member.length; i++) {
        ...
        if (!args[0]) {
        return conn.reply(m.chat, `*${emojis} Utilice el comando con opciones:*\n1. \`.fantasmas list\` para etiquetar los miembros inactivos\n2. \`.fantasmas kick\` para expulsar a miembros inactivos`, m)
    }

    if (args[0] === 'list') {
        if (total === 0) return conn.reply(m.chat, `*${emojis} Este grupo es activo*`, m)
    ...
    if (args[0] === 'kick') {
        if (total === 0) return conn.reply(m.chat, `*${emojis} No hay nasesidad de eliminar a nadie, porque este grupo es activo*`, m)
